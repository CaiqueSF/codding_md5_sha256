...
def calculaHashArquivoTexto():
    ...
    if texto:
        ...
        textoCoding = texto.encode('utf-8')
...
def check_MD5():
    ...
    retornoCheck = ttk.Label(janela, text='           MD5 HEXADECIMAL           ', font=(
        "Arial", 15, 'bold'), foreground='green')
...
def check_SHA256():
    checkOK = ttk.Label(janela, image=images['check'])
    checkOK.grid(row=5, column=0, pady=10)
    retornoCheck = ttk.Label(janela, text='           SHA-256 HEXADECIMAL           ',
                             font=("Arial", 15, 'bold'), foreground='green')
    retornoCheck.grid(row=5, column=1, padx=3, pady=5)
    atualizaJanela()
...
def NOcheck():
    ...
    retornoCheck = ttk.Label(janela, text='NÃO CORREESPONDENTE', font=(
        "Arial", 15, 'bold'), foreground='red')
    retornoCheck.grid(row=5, column=1, padx=3, pady=5)
    atualizaJanela()
...
def checking():
    ...
    if 'caminho' in globals():
        ...
        if verificaCheck == md5_hex:
            check_MD5()
        elif verificaCheck == sha256_hex:
            check_SHA256()
        else:
            NOcheck()
    ...
    # Autenticidade de hash para textos
        ...
        if verificaCheck == h_MD5.hexdigest():
            check_MD5()
        elif verificaCheck == h_SHA256.hexdigest():
            check_SHA256()
        else:
            NOcheck()
...
janela.configure(bg='black')  # Azul Escuro = black
janela.geometry('695x255')
janela.minsize(695, 255)  # Tamanho mínimo da janela
janela.resizable(True, False)  # Desabilita redimensionamento (horizontal, vertical)
janela.iconbitmap('images/cripto.ico')
...
botao_selecionar = ttk.Button(
    janela, text="        CARREGAR ARQUIVO        ", style='TButton', command=calculaHashArquivo)
botao_selecionar.grid(row=0, column=1, pady=10)

# Entrada de Texto
criptografaTexto = ttk.Button(
    janela, text="TEXTO", style='TButton', command=calculaHashArquivoTexto)
...
# SAÍDA MD5
labelMD5_HEX = ttk.Label(janela, text='MD5 (hex):', font=(
    "Arial", 15, 'bold'), foreground='white')
labelMD5_HEX.grid(row=2, column=0, padx=3, pady=5)
saidaMD5_HEX = ttk.Label(janela, text='', font=(
    "Arial", 17, 'bold'), foreground='white')
saidaMD5_HEX.grid(row=2, column=1, padx=3, pady=5)
saidaMD5_HEX.bind(
    "<Button-1>", lambda e: pyperclip.copy(saidaMD5_HEX.cget("text")))

# SAÍDA SHA-256
labelSHA256_HEX = ttk.Label(
    janela, text='SHA-256 (hex):', font=("Arial", 15, 'bold'), foreground='white')
labelSHA256_HEX.grid(row=3, column=0, padx=3, pady=5)
saidaSHA256_HEX = ttk.Label(janela, text='', font=(
    "Arial", 17, 'bold'), foreground='white')
saidaSHA256_HEX.grid(row=3, column=1, padx=3, pady=5)
saidaSHA256_HEX.bind(
    "<Button-1>", lambda e: pyperclip.copy(saidaSHA256_HEX.cget("text")))

# Conferência da CRIPTOGRAFIA
buttonCheck = ttk.Button(janela, text="CHECK",
                         style='TButton', command=checking)
...
# Dicionário com valores nulos para armazenar imagens
images = {}
images['check'] = PhotoImage(file='images/check.png')
images['check'] = images['check'].subsample(
    8, 8)  # Reduz o tamanho para 15x15 pixels
images['no_check'] = PhotoImage(file='images/noCheck.png')
images['no_check'] = images['no_check'].subsample(
    8, 8)  # Reduz o tamanho para 15x15 pixels
